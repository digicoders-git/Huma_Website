...
const SpecialityDetail = () => {
  ...
  const fetchDetail = async () => {
    try {
      const res = await fetch(`${API_URL}/speciality/${id}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4">
          <h2 className="text-xl font-bold text-primary italic uppercase">Speciality not found</h2>
          <Link to="/specialities" className="text-secondary font-bold underline uppercase tracking-widest text-xs">Back to Specialities</Link>
        </div>
      </div>
    );
  }
      ...
      <section className="py-6 px-4 md:px-8">
        <div className="max-w-4xl mx-auto">
          <div className="relative rounded-2xl overflow-hidden bg-slate-100 mb-6">
            <img src={getFullImageUrl(treatment.image)} alt={treatment.name} className="w-full h-[300px] md:h-[400px] object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-primary/80 to-transparent" />
            <div className="absolute bottom-0 left-0 right-0 p-6">
              <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">{treatment.name}</h1>
          ...
          <div className="space-y-6">
            ...
            {/* When Recommended */}
            {treatment.whenRecommended && treatment.whenRecommended.length > 0 && (
              <div className="space-y-3">
